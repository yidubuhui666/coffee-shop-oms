# Retry with longer timeout and sequential
$ua = "Mozilla/5.0 Chrome/120"
$dest = "C:\Users\xuyq\coffee-shop-oms\src\static\img\products".Replace("xuyq", [char]0x8BB8 + "yq")

$products = @(
    @{ file = "coconut-cold-brew.jpg"; seed = 161; prompt = "An iced coconut cold brew coffee in a tall clear glass with ice cubes, dark cold brew coffee mixed with creamy coconut milk creating layers, fresh coconut chunks beside the glass, professional product photography, natural light, wooden table, realistic" }
...
foreach ($p in $products) {
    ...
    if ((Test-Path $target) -and (Get-Item $target).Length -gt 20000) {
        ...
        Write-Host "SKIP $($p.file) (already exists)"
        continue
    }
    ...
    Start-Sleep -Seconds 2  # Be polite between requests
}
